...
class Cifar10DataReader():
    ...
    def unpickle(self, f):
        ...
        fo = open(f, 'rb')# huo qu wen jian 

        d = pickle.load(fo)#jiang shu ju jie xi wei python diu xaing

        fo.close()#guan bi wen jian

        return d # fan hui huo qu de wen jian

 

    def next_train_data(self, batch_size=20):

        assert 300 % batch_size == 0, "10000%batch_size!=0"
        ...
        if self.read_next:
            ...
            f = os.path.join(self.cifar_folder, "car_train_tfrecords")# jiang duo ge lu jin zu he fan hui
            ...
            datum = self.data_label_train[self. batch_size:(batch_size]rdata, rlabel = self._decode(datum, self.onehot)
    ...
    def next_test_data(self, batch_size=20):
